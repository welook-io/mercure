...
export async function generateInvoicePdf(params: InvoicePdfParams): Promise<Buffer> {
  ...
  const qrData = {
    ...
    ptoVta: parseInt(params.invoiceNumber.split('-')[0]),
    tipoCmp: params.invoiceType === 'A' ? 1 : 6,
    nroCmp: parseInt(params.invoiceNumber.split('-')[1]),
    importe: params.total,
    moneda: 'PES',
    ctz: 1,
    tipoDocRec: 80,
    nroDocRec: parseInt(params.clienteCuit.replace(/-/g, '')),
    tipoCodAut: 'E',
...
<body>
  <div class="header">
    ...
    <div class="header-center">
      <div class="tipo-factura">${params.invoiceType}</div>
      <div class="codigo-tipo">COD. ${params.invoiceType === 'A' ? '01' : '06'}</div>
    </div>
    <div class="header-right">
      <div class="factura-titulo">FACTURA</div>
      <div class="factura-numero">Nº ${params.invoiceNumber}</div>
  ...
  <div class="cliente-box">
    ...
    <div class="cliente-row">
      ...
      <span>${params.clienteNombre}</span>
    </div>
    <div class="cliente-row">
      <span class="cliente-label">CUIT:</span>
      <span>${params.clienteCuit}</span>
    </div>
    <div class="cliente-row">
  ...
  <table class="detalle-table">
    ...
    <tbody>
      <tr>
        ...
        <td class="text-right">${formatCurrency(params.neto)}</td>
        <td class="text-right">${formatCurrency(params.neto)}</td>
      </tr>
    </tbody>
  </table>
  ...
  <div class="totales-box">
    <div class="totales-content">
      <div class="total-row">
        ...
        <span>${formatCurrency(params.neto)}</span>
      </div>
      <div class="total-row">
        <span>IVA 21%:</span>
        <span>${formatCurrency(params.iva)}</span>
      </div>
  ...
  <div class="cae-box">
    <div class="cae-info">
      ...
      <div class="cae-numero">${params.cae}</div>
